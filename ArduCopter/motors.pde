...
static void arm_motors_check()
{
    ...
    if (g.rc_3.control_in > 0) {
        if (arming_counter) {
            log_dbg() << "arm_motors_check: abort (throttle is up) " << g.rc_3.control_in;
        }
        arming_counter = 0;
        return;
    }
    ...
    if (!allow_arming) {
        if (arming_counter) {
            log_dbg() << "arm_motors_check: abort - not allowed";
        }
        arming_counter = 0;
        return;
    }
    ...
    if (tmp > 4000) {
        ...
        if( arming_counter <= AUTO_TRIM_DELAY ) {
            arming_counter++;
            log_dbg() << "arming: " << (int)arming_counter << " " << tmp;
        }
        ...
        if( arming_counter <= DISARM_DELAY ) {
            arming_counter++;
            log_dbg() << "disarming: " << (int)arming_counter << " " << tmp;
        }
        ...
        if (arming_counter) {
            log_dbg() << "arm_motors_check: reset  arming_counter:" << arming_counter << " yaw:" << (int)tmp;
        }
        arming_counter = 0;
    }
}
