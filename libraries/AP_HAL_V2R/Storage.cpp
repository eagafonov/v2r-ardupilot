...
#define STORAGE_DIR "APM"
#define STORAGE_FILE STORAGE_DIR "/" SKETCHNAME ".stg"

extern const AP_HAL::HAL& hal;

void V2RStorage::_storage_create(void)
{
	...
	for (uint16_t loc=0; loc<sizeof(_buffer); loc += V2R_STORAGE_MAX_WRITE) {
		if (write(fd, &_buffer[loc], V2R_STORAGE_MAX_WRITE) != V2R_STORAGE_MAX_WRITE) {
...
void V2RStorage::_storage_open(void)
{
	if (_initialised) {
		return;
	}
 ...
 */
void V2RStorage::_mark_dirty(uint16_t loc, uint16_t length)
{
	uint16_t end = loc + length;
	while (loc < end) {
		uint8_t line = (loc >> V2R_STORAGE_LINE_SHIFT);
		_dirty_mask |= 1 << line;
		loc += V2R_STORAGE_LINE_SIZE;
	}
}

uint8_t V2RStorage::read_byte(uint16_t loc) 
{
...
uint16_t V2RStorage::read_word(uint16_t loc) 
{
	uint16_t value;
...
uint32_t V2RStorage::read_dword(uint16_t loc) 
{
	uint32_t value;
	if (loc >= sizeof(_buffer)-(sizeof(value)-1)) {
		return 0;
...
void V2RStorage::read_block(void *dst, uint16_t loc, size_t n) 
{
...
void V2RStorage::write_byte(uint16_t loc, uint8_t value) 
{
	if (loc >= sizeof(_buffer)) {
		return;
	}
...
void V2RStorage::write_word(uint16_t loc, uint16_t value) 
{
	if (loc >= sizeof(_buffer)-(sizeof(value)-1)) {
		return;
	}
...
void V2RStorage::write_dword(uint16_t loc, uint32_t value) 
{
	if (loc >= sizeof(_buffer)-(sizeof(value)-1)) {
		return;
	}
...
void V2RStorage::write_block(uint16_t loc, const void *src, size_t n) 
{
	if (loc >= sizeof(_buffer)-(n-1)) {
		return;
	}
...
void V2RStorage::_timer_tick(void)
{
	...
	uint8_t i, n;
	for (i=0; i<V2R_STORAGE_NUM_LINES; i++) {
		if (_dirty_mask & (1<<i)) {
			break;
		}
	}
	if (i == V2R_STORAGE_NUM_LINES) {
		// this shouldn't be possible
		return;
	}
	uint32_t write_mask = (1U<<i);
	// see how many lines to write
	for (n=1; (i+n) < V2R_STORAGE_NUM_LINES && 
		     n < (V2R_STORAGE_MAX_WRITE>>V2R_STORAGE_LINE_SHIFT); n++) {
	 ...
	 */
	if (lseek(_fd, i<<V2R_STORAGE_LINE_SHIFT, SEEK_SET) == (i<<V2R_STORAGE_LINE_SHIFT)) {
		_dirty_mask &= ~write_mask;
		if (write(_fd, &_buffer[i<<V2R_STORAGE_LINE_SHIFT], n<<V2R_STORAGE_LINE_SHIFT) != n<<V2R_STORAGE_LINE_SHIFT) {
