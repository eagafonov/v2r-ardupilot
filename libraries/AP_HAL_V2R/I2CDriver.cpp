...
/*
  constructor
 */
V2RI2CDriver::V2RI2CDriver(AP_HAL::Semaphore* semaphore, const char *device) : 
...
void V2RI2CDriver::setTimeout(uint16_t ms) 
{
    // unimplemented
}

void V2RI2CDriver::setHighSpeed(bool active) 
{
    // unimplemented    
}

uint8_t V2RI2CDriver::write(uint8_t addr, uint8_t len, uint8_t* data)
{
    if (!set_address(addr)) {
        return 1;
    }
...
uint8_t V2RI2CDriver::writeRegisters(uint8_t addr, uint8_t reg,
...
uint8_t V2RI2CDriver::writeRegister(uint8_t addr, uint8_t reg, uint8_t val)
{
    if (!set_address(addr)) {
        return 1;
    }
...
uint8_t V2RI2CDriver::read(uint8_t addr, uint8_t len, uint8_t* data)
{
    if (!set_address(addr)) {
        return 1;
    }
...
uint8_t V2RI2CDriver::readRegisters(uint8_t addr, uint8_t reg,
                                      uint8_t len, uint8_t* data)
{
    if (!set_address(addr)) {
        return 1;
...
uint8_t V2RI2CDriver::lockup_count() 
{
    return 0;
}
